...
export default function App() {
  ...
  useEffect(() => {
    ...
    const messagesRef = collection(db, `users/${auth.currentUser.uid}/chatHistory`);
  ...
  const addMessageToFirestore = async (message: Omit<ChatMessage, "id" | "createdAt">) => {
    if (!auth.currentUser) return;
    const messagesRef = collection(db, `users/${auth.currentUser.uid}/chatHistory`);
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    await addMessageToFirestore({ sender: "user", text: finalTranscript });
    ...
    if (commandResult.isBrowserAction) {
      ...
      await addMessageToFirestore({ sender: "zoya", text: responseText });
      ...
      responseText = await getZoyaResponse(finalTranscript, messages); // Using messages directly from state
      await addMessageToFirestore({ sender: "zoya", text: responseText });
  ...
  }, [isMuted, isSessionActive, messages]);
    ...
    if (isSessionActive) {
      ...
      try {
        ...
        session.onMessage = (sender, text) => {
          addMessageToFirestore({ sender, text });
        };
